...
export default function BoardsFailureBanner({ errors, requestedAshbySlugs }) {
  ...
  return (
    <div className="fade" style={{ ...cardStyle, borderColor: COLORS.danger, background: COLORS.dangerSoft, padding: '10px 14px', marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <span style={{ fontSize: 12, color: COLORS.text }}>
          <span aria-hidden="true" style={{ color: COLORS.danger, fontFamily: FONTS.mono, marginRight: 8 }}>!</span>
        ...
        <div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px solid ${COLORS.border}` }}>
          {errors.map((e, i) => (
            <div key={`${e.source}-${e.slug}-${i}`} style={{ fontSize: 11, color: COLORS.textSecondary, padding: '2px 0' }}>
              <span style={{ fontFamily: FONTS.mono, color: COLORS.text }}>{e.source}/{e.slug}</span>
              {': '}{e.reason}
            </div>
          ))}
        </div>
